...
DECLARE_EVENT_CLASS(smb3_lock_class,
	TP_PROTO(unsigned int xid,
		...
		__u32	flags,
		int	rc),
	TP_ARGS(xid, fid, tid, sesid, offset, len, flags, rc),
	TP_STRUCT__entry(
		__field(unsigned int, xid)
		__field(__u64, fid)
		__field(__u32, tid)
	...
	TP_printk("xid=%u sid=0x%llx tid=0x%x fid=0x%llx offset=0x%llx len=0x%llx flags=0x%x rc=%d",
		__entry->xid, __entry->sesid, __entry->tid, __entry->fid,
		__entry->offset, __entry->len, __entry->flags, __entry->rc)
)
...
DECLARE_EVENT_CLASS(smb3_open_done_class,
	TP_PROTO(unsigned int xid,
		...
		int	create_options,
		int	desired_access,
		__u8	rsp_oplock_level,
		__u8	oplock),
	TP_ARGS(xid, fid, tid, sesid, create_options, desired_access,
		rsp_oplock_level, oplock),
	TP_STRUCT__entry(
		__field(unsigned int, xid)
		__field(__u64, fid)
		__field(__u32, tid)
	...
	TP_fast_assign(
		...
		__entry->create_options = create_options;
		__entry->desired_access = desired_access;
		__entry->rsp_oplock_level = rsp_oplock_level;
		__entry->oplock = oplock;
	),
	TP_printk("xid=%u sid=0x%llx tid=0x%x fid=0x%llx cr_opts=0x%x des_access=0x%x op_level=0x%x state=0x%x",
		__entry->xid, __entry->sesid, __entry->tid, __entry->fid,
		__entry->create_options, __entry->desired_access,
		__entry->rsp_oplock_level, __entry->oplock)
)
...
DEFINE_EVENT(smb3_open_done_class, smb3_##name,  \
	TP_PROTO(unsigned int xid,		\
		...
		__u8	rsp_oplock_level,	\
		__u8	oplock),		\
	TP_ARGS(xid, fid, tid, sesid, create_options, desired_access, rsp_oplock_level, oplock))
